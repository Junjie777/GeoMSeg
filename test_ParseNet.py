...
class Args(object):
    input_image_path = 'image/map05.png'  # image/coral.jpg image/tiger.jpg
    train_epoch = 2 ** 6
    mod_dim1 = 64
    mod_dim2 = 45
    gpu_id = 0
...
class MyNet(nn.Module):
    def __init__(self, inp_dim, mod_dim1, mod_dim2):
        super(RCUnit, self).__init__()

        self.conv1 = nn.Conv2d(
            inp_dim, mod_dim1, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv2 = nn.Conv2d(
            mod_dim1, mod_dim2, kernel_size=3, stride=1, padding=1, bias=False)
...
def run():
    ...
    args = Args()
    # 为所有的GPU设置随机数种子 保证每次运行.py文件时，生成的随机数都是固定的，这样每次的实验结果也就一致了。
    torch.cuda.manual_seed_all(1943)
    # numpy内部的随机种子
    np.random.seed(1943)
    # 设置当前使用的GPU设备仅为0号 要将该句放在所有访问GPU的代码之前
    os.environ['CUDA_VISIBLE_DEVICES'] = str(args.gpu_id)  # choose GPU:0
    # 读取图片 返回的是一个三维数组(H,W,C)
